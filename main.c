...
// #define NOB_TEMP_CAPACITY (100*1024*1024)
#include "nob.h"
...
typedef struct {
  ...
  uint64_t *node_year;
  uint64_t *edge_index; // Graph connectivity in COO format with shape [2, num_edges]
  double   *x; // Node feature matrix with shape [num_nodes, num_node_features] (features)
  uint64_t *y; // node-level targets of shape [num_nodes, num_label_classes] or graph-level targets of shape [1, num_label_classes] (labels)
} ogb_arxiv_t;

#define CHUNK 0x1000 // 4kb window size

String_View read_gz(char* file_path, String_Builder *sb)
{
...
int main(void)
{
  ...
  sv = read_gz(RAW_PATH"/node-label.csv.gz", &sb);
  ...
  sv = read_gz(RAW_PATH"/node-feat.csv.gz", &sb);
